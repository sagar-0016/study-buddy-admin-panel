...
const seedDoubts = async () => {
    ...
    const sampleDoubts = [
        {
            text: "In the chapter on Rotational Motion, I'm confused about the parallel axis theorem. When is it better to use it over the perpendicular axis theorem? The textbook explanation is a bit vague.",
            subject: "Physics",
        ...
        {
            text: "What's the best way to remember all the exceptions in the p-block elements for JEE? Specifically for the Boron and Carbon families. Are there any mnemonics?",
            subject: "Chemistry",
            isAddressed: true,
            isCleared: false,
            createdAt: Timestamp.fromDate(new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)), // 2 days ago
            addressedText: "A great mnemonic for p-block group 13 (Boron family) is: 'B.Al.Ga.In.Tl' -> 'Boring Aluminum Gallaries In Thailand'. For Group 14 (Carbon family): 'C.Si.Ge.Sn.Pb' -> 'Chemists Sir Give Sanskrit Problems'. Hope this helps!"
        },
        {
            text: "I'm struggling with integrating functions involving inverse trigonometric substitutions. For example, integrating sqrt(a^2 - x^2). Can you provide a step-by-step example?",
            subject: "Maths",
            isAddressed: false,
            isCleared: false,
        ...
        {
            text: "This doubt has been addressed and also cleared by the user.",
            subject: "Physics",
            isAddressed: true,
            isCleared: true,
            createdAt: Timestamp.fromDate(new Date(Date.now() - 3 * 24 * 60 * 60 * 1000)), // 3 days ago
            addressedText: "This has been addressed. The user confirmed they understood the solution."
        }
    ];

    sampleDoubts.forEach(doubt => {
        const docRef = doc(doubtsRef);
        batch.set(docRef, doubt);
    });

    await batch.commit();
    console.log("Sample doubts seeded.");
}
...
export const getDoubts = async (status: 'pending' | 'addressed' = 'pending'): Promise<Doubt[]> => {
  try {
    ...
    // Temporarily removed seeding logic as per user instruction
    // const initialSnapshot = await getDocs(query(doubtsRef, limit(1)));
...
const uploadVideo = async (file: File): Promise<string> => {
    ...
    // Set metadata to make the file publicly readable
    const metadata = {
 ...
 */
export const updateCardInDeck = async (deckId: string, cardId: string, data: Omit<Partial<Flashcard>, 'id'>): Promise<void> => {
    try {
