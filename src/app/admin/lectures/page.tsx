...
import { getLectures, addLecture } from '@/lib/admin/firebase-actions';
import type { Lecture } from '@/lib/admin/types';
...
const UploadLectureDialog = ({ onUploadComplete }: { onUploadComplete: () => void }) => {
    ...
    const canSubmit = useMemo(() => {
        return !!videoFile && title && subject;
    }, [videoFile, title, subject]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            videoElement.onloadedmetadata = () => {
                ...
                const videoDuration = Math.round(videoElement.duration / 60);
                const hours = Math.floor(videoDuration / 60);
                ...
                <DialogHeader>
                 ...
                 <div className="grid gap-4 py-4">
                    ...
                    <div className="grid w-full items-center gap-1.5">
                     ...
                     <div className="grid w-full items-center gap-1.5">
                        <Label htmlFor="video">Video File <span className="text-destructive">*</span></Label>
                        <Input id="video" type="file" onChange={handleFileChange} accept="video/*"/>
                    </div>
                </div>
                <DialogFooter>
...
export default function LecturesPage() {
  ...
  const filteredLectures = useMemo(() => {
    ...
    return lectures.filter(lecture => 
      ...
      (lecture.description && lecture.description.toLowerCase().includes(lowercasedTerm)) ||
      lecture.subject.toLowerCase().includes(lowercasedTerm) ||
      (lecture.channel && lecture.channel.toLowerCase().includes(lowercasedTerm))
    );
  }, [lectures, searchTerm]);
    ...
    <div className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Browse, search, and upload lectures for the user-facing app.
          </CardDescription>
        </CardHeader>
        <CardContent>
